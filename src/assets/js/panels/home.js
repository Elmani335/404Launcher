...
class Home {
    ...
    async init(config) {
        ...
        // Setup global error handler for JSON parsing errors
    ...
    setupErrorHandlers() {
        ...
        window.addEventListener('unhandledrejection', (event) => {
            ...
            if (event.reason && event.reason.message) {
                ...
                if (errorMessage.includes('unexpected token') && errorMessage.includes('<')) {
                    console.error('[Error Handler] Detected HTML response instead of JSON');
                    this.showAlert('Connection Error', 'Server returned an invalid response. Please check your connection and try again.');
                ...
                // Check for other common server errors
                if (errorMessage.includes('failed to fetch') || errorMessage.includes('network error')) {
                    this.showAlert('Network Error', 'Unable to connect to server. Please check your internet connection.');
                    event.preventDefault();
                    return;
                }
            }
        ...
        console.log('[Error Handler] Global error handlers setup complete');
    }
    ...
    async launchGame() {
        ...
        console.log('[Launch] DOM Elements found:', {
            playButton: !!playInstanceBTN,
            progressContainer: !!infoStartingBOX,
            progressText: !!infoStarting,
        ...
        let opt = {
            ...
            timeout: 10000,
            path: configClient.game_config?.game_path || `${process.env.APPDATA || process.env.HOME}/.minecraft`,
        ...
        // Keep button visible but disable it during launch
        if (playInstanceBTN) {
            playInstanceBTN.disabled = true
            playInstanceBTN.classList.add('launching')
        }
        ...
        launch.on('data', (e) => {
            ...
            if (infoStarting) infoStarting.innerHTML = `Game is running...`
            if (playInstanceBTN) {
                playInstanceBTN.classList.remove('launching')
                playInstanceBTN.classList.add('running')
            }
            console.log(e);
        })
        ...
        launch.on('close', code => {
            ...
            if (playInstanceBTN) {
                ...
                playInstanceBTN.classList.remove('launching', 'running')
            }
        ...
        launch.on('error', err => {
            ...
            if (playInstanceBTN) {
                playInstanceBTN.disabled = false
                playInstanceBTN.classList.remove('launching', 'running')
                playInstanceBTN.classList.add('error')
            }
            ...
            console.error('[Launch] Error in launchGame:', error);
            
            // Check for specific JSON parsing errors
            if (error.message && error.message.toLowerCase().includes('unexpected token') && error.message.toLowerCase().includes('<')) {
                this.showAlert('Connection Error', 'Server returned an invalid response. Please check your connection and try again.');
            } else {
                this.showAlert('Launch Error', error.message || 'Failed to start launch process');
            }
